...
_IOTDATA_CLIENT = None

def _iot_publish(topic: str, payload: dict, qos: int = 0):
    # Publishes to AWS IoT Core using Lambda execution role (SigV4 via boto3)
    global _IOTDATA_CLIENT
    if _IOTDATA_CLIENT is None:
        region = os.getenv("AWS_REGION", "eu-central-1")
        _IOTDATA_CLIENT = boto3.client("iot-data", region_name=region)
    _IOTDATA_CLIENT.publish(
        topic=topic,
        qos=qos,
...
def lambda_handler(event, context=None):
    ...
    print('TOPIC_NAME:', event.get('topic_name'))
    """
    AWS Lambda handler.
    """
    global _INDEX_CACHE
    ...
    wspd, wdir_from, wind_ts = get_wind_at_latlon_time(
        alert["lat"],
        alert["lon"],
        alert["timestamp"],
    )
    ...
    record = match_windfield(_INDEX_CACHE, wspd, wdir_from, domain_hint=None)
