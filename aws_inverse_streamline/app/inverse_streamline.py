...
def run_inverse_streamline(*,
    ...
    with rasterio.open(dsm_path) as dsm_ds, rasterio.open(vel_path) as vel_ds, rasterio.open(ang_path) as ang_ds:
        ...
        return {
            ...
            "inputs": {
                "pm25_obs_ugm3": pm25_obs,
                "sensor_lat": sensor_lat,
                "sensor_lon": sensor_lon,
                "model_crs": model_crs,
            },
            "sample_at_sensor": {
                "wind_speed_ms": float(vel_val),
                "wind_dir_from_deg": float(ang_val),
                "wind_dir_to_deg": float(dir_to_deg),
                "terrain_height_m": float(z_val),
            },
            "decay": {
                "C0_ref": params.C0_ref,
                "C_ref": params.C_ref,
                "d_ref_m": params.d_ref_m,
                "k_1_per_m": float(k),
                "C0_min": float(C0_min),
                "C0_max": float(C0_max),
            },
